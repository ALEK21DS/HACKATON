    ...
    <div className="page-container">
      <header style={{ marginBottom: '2.5rem' }}>
        ...
        <p style={{ color: '#8C8C8C', fontSize: '0.9rem' }}>
          Configura y monitorea la conexión con los motores de Meta y Google Cloud.
        </p>
      </header>
        ...
        <>
          {status && (
            <div style={{ 
              ...
              <div style={{ flex: 1 }}>
                <span style={{ fontSize: '0.7rem', color: '#8C8C8C', fontWeight: 700, textTransform: 'uppercase', display: 'block', marginBottom: '0.5rem' }}>WhatsApp (Meta)</span>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <div style={{ width: 8, height: 8, borderRadius: '50%', background: status.hasWhatsappToken ? '#4ADE80' : '#EF4444', boxShadow: `0 0 10px ${status.hasWhatsappToken ? 'rgba(74, 222, 128, 0.4)' : 'rgba(239, 68, 68, 0.4)'}` }}></div>
                  <span style={{ fontSize: '0.9rem', color: '#F2F2F2', fontWeight: 600 }}>{status.hasWhatsappToken ? 'ACTIVO' : 'PENDIENTE'}</span>
                  <span style={{ fontSize: '0.75rem', color: '#8C8C8C' }}>{status.whatsappPhoneNumberId ? `(ID: ${status.whatsappPhoneNumberId})` : ''}</span>
                </div>
              </div>
              <div style={{ width: '1px', background: 'rgba(64,64,64,0.3)' }}></div>
              <div style={{ flex: 1 }}>
                <span style={{ fontSize: '0.7rem', color: '#8C8C8C', fontWeight: 700, textTransform: 'uppercase', display: 'block', marginBottom: '0.5rem' }}>Inteligencia Artificial (Google)</span>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <div style={{ width: 8, height: 8, borderRadius: '50%', background: status.hasGeminiKey ? '#4ADE80' : '#EF4444', boxShadow: `0 0 10px ${status.hasGeminiKey ? 'rgba(74, 222, 128, 0.4)' : 'rgba(239, 68, 68, 0.4)'}` }}></div>
                  <span style={{ fontSize: '0.9rem', color: '#F2F2F2', fontWeight: 600 }}>{status.hasGeminiKey ? 'ACTIVO' : 'PENDIENTE'}</span>
                  <span style={{ fontSize: '0.75rem', color: '#8C8C8C' }}>{status.hasGeminiKey ? '(Modelo Sincronizado)' : '(Sin configurar)'}</span>
                </div>
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(64,64,64,0.3)', borderRadius: 16, padding: '1.5rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
                ...
                <h3 style={{ fontSize: '1rem', fontWeight: 700, color: '#F2F2F2', margin: 0 }}>Meta for Developers (WhatsApp)</h3>
              </div>

              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <label style={{ fontSize: '0.8rem', color: '#8C8C8C', fontWeight: 600, textTransform: 'uppercase' }}>WhatsApp Access Token</label>
                  <div style={{ position: 'relative' }}>
                    <input
                      ...
                      placeholder="••••••••••••••••••••••••••••••••"
                      style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(64,64,64,0.3)', borderRadius: 10, padding: '0.85rem 3rem 0.85rem 1rem', color: '#F2F2F2', outline: 'none' }}
                    />
                    <button type="button" onClick={() => setShowWaToken(!showWaToken)} style={{ position: 'absolute', right: '1rem', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: '#8C8C8C', cursor: 'pointer' }}>
                      {showWaToken ? <EyeOffIcon className="w-5 h-5" /> : <EyeIcon className="w-5 h-5" />}
                    </button>
                  </div>
                  <span style={{ fontSize: '0.7rem', color: '#666' }}>Usa un token de acceso permanente de tu App en Meta.</span>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                    <label style={{ fontSize: '0.8rem', color: '#8C8C8C', fontWeight: 600, textTransform: 'uppercase' }}>Phone Number ID</label>
                    <input
                      type="text"
                      value={waPhoneId}
                      onChange={(e) => setWaPhoneId(e.target.value)}
                      placeholder="Ej: 1029384756..."
                      style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(64,64,64,0.3)', borderRadius: 10, padding: '0.85rem 1rem', color: '#F2F2F2', outline: 'none' }}
                    />
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                    <label style={{ fontSize: '0.8rem', color: '#8C8C8C', fontWeight: 600, textTransform: 'uppercase' }}>WABA ID (Opcional)</label>
                    <input
                      type="text"
                      value={waWaba}
            ...
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(64,64,64,0.3)', borderRadius: 16, padding: '1.5rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
                ...
                <h3 style={{ fontSize: '1rem', fontWeight: 700, color: '#F2F2F2', margin: 0 }}>Google AI Studio (Gemini)</h3>
              </div>

              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <label style={{ fontSize: '0.8rem', color: '#8C8C8C', fontWeight: 600, textTransform: 'uppercase' }}>Gemini API Key</label>
                <div style={{ position: 'relative' }}>
                  <input
                    ...
                    style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(64,64,64,0.3)', borderRadius: 10, padding: '0.85rem 3rem 0.85rem 1rem', color: '#F2F2F2', outline: 'none' }}
                  />
                  <button type="button" onClick={() => setShowGeminiKey(!showGeminiKey)} style={{ position: 'absolute', right: '1rem', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: '#8C8C8C', cursor: 'pointer' }}>
                    {showGeminiKey ? <EyeOffIcon className="w-5 h-5" /> : <EyeIcon className="w-5 h-5" />}
                  </button>
                </div>
                <span style={{ fontSize: '0.7rem', color: '#666' }}>Consigue tu API Key en Google AI Studio para activar las respuestas inteligentes.</span>
              </div>
            </div>

            {err && <div style={{ padding: '1rem', background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.2)', borderRadius: 12, color: '#EF4444', fontSize: '0.85rem' }}>{err}</div>}
            {msg && <div style={{ padding: '1rem', background: 'rgba(74, 222, 128, 0.1)', border: '1px solid rgba(74, 222, 128, 0.2)', borderRadius: 12, color: '#4ADE80', fontSize: '0.85rem' }}>{msg}</div>}
          ...
          {/* Lead Detection Card */}
          <div style={{ marginTop: '2rem', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(64,64,64,0.3)', borderRadius: 16, padding: '1.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
              <div style={{ padding: '0.5rem', background: 'rgba(239, 68, 68, 0.1)', borderRadius: 10, color: '#EF4444' }}>
                <RobotIcon className="w-5 h-5" />
              </div>
              <div style={{ flex: 1 }}>
                <h3 style={{ fontSize: '1rem', fontWeight: 700, color: '#F2F2F2', margin: 0 }}>Detección Automática de Nuevos Leads</h3>
              </div>
              <div 
              ...
              >
                <div style={{ width: '24px', height: '24px', borderRadius: '50%', background: 'white', position: 'absolute', top: '4px', left: leadConfig.enabled ? '36px' : '4px', transition: 'all 0.3s ease' }}></div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <label style={{ fontSize: '0.8rem', color: '#8C8C8C', fontWeight: 600, textTransform: 'uppercase' }}>Mensaje de detección</label>
              <textarea
                ...
                rows={4}
                style={{ 
                  width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(64,64,64,0.3)', 
                  borderRadius: 10, padding: '0.85rem 1rem', color: '#F2F2F2', outline: 'none', 
                  fontSize: '0.9rem', resize: 'vertical', fontFamily: 'inherit',
                  opacity: leadConfig.enabled ? 1 : 0.4, boxSizing: 'border-box',
                }}
            ...
            {leadConfig.enabled && leadConfig.autoMessage.trim() && (
              <div style={{
                ...
                <div style={{
                  ...
                  <span style={{
                    fontSize: '0.65rem', color: '#8C8C8C', fontWeight: 800,
                ...
                <div style={{
                  ...
                  <span>
                    El sistema comparará este texto con cada mensaje entrante usando similitud inteligente (&ge; 75%).
                  </span>
                </div>
              </div>
            )}
